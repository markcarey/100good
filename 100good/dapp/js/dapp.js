...
const chainId = '0xa4ec'; // celo

const firebaseConfig = {
    apiKey: "AIzaSyD7NC3RTzEW-tqW-EcaG3wcSFnUr1WkBGo",
    authDomain: "hundred-good.firebaseapp.com",
    projectId: "hundred-good",
    storageBucket: "hundred-good.appspot.com",
    messagingSenderId: "495079767923",
    appId: "1:495079767923:web:3c0a95dbd00d3e494e8f06"
  };
...
const wcProjectId = '3a553649ee03bf5b1c9c7b3abf566d5c';
                ...
                querySnapshot.forEach((doc) => {
                    ...
                    doc.ref.collection("wallet").doc("balances")
                        .onSnapshot((doc) => {
                            if (doc.exists) {
                                balances = doc.data();
                                var goodBalance = parseFloat(ethers.utils.formatEther(ethers.BigNumber.from(balances.good)));
                                $("#good-balance").text(goodBalance.toFixed(0));
                                $("#wallet .good").text(goodBalance.toFixed(3));
                            }
                        });

                    $("#safeaddress").text(user.safeAddress);
                    $(".onramp.step-1").addClass("hidden");
                    if (meta.safeDeployed == false) {
                        $("#good-balance").text(0);  // not real tokens (yet)
                        if (meta.fundlink) {
                            $(".wallet-link").attr("href", meta.fundlink);
                            // TODO: render modal or message to fund Safe
                        }
                        updateBalances();
                    } else {
                        $(".wallet-link").attr("href", `https://app.safe.global/balances?safe=celo:${user.safeAddress}`);
                        updateBalances();
                    }
                    updateFollowButtons();
...
async function updateBalances() {
    ...
    if ("good" in balances) {
        var goodBalance = parseFloat(ethers.utils.formatEther(ethers.BigNumber.from(balances.good)));
        if (goodBalance > 8) {
            $(".onramp.step-2").addClass("hidden");
            $(".onramp.step-3").removeClass("hidden");
            $("#good-balance").text(goodBalance.toFixed(0));
            $("#wallet .good").text(goodBalance.toFixed(3));
        } else {
            $(".onramp.step-2").removeClass("hidden");
        }
    }
    $("#onramp-balance").text("Check Balance");
}
...
async function fund() {
    const headers = await getHeaders();
    const res = await fetch('/api/fundlink', { 
        method: 'GET', 
        headers: new Headers(headers)
    });
...
$( document ).ready(function() {
    ...
    $("#login, #onramp-login").click(async function (event) {
    ...
    $("#onramp-fund").click(async function (e) {
        if ("fundlink" in loggedInUser) {
            window.open(loggedInUser.fundlink, '_blank');
        }
        return false;
    });

    $("#onramp-balance").click(async function (e) {
        $(this).text("Checking...");
        updateBalances();
        return false;
    });
    ...
    $( "#feed-posts, #story-modal" ).on( "click", ".stream", async function(e) {
        e.preventDefault();
        console.log("stream!");
        var data = {};
        data.id = $(this).data('id');
        stream(data);
        $(this).find('i').css("color", "red");
        $(this).find('.stream-button-text').text(" Streamed!");
        return false;
    });
    ...
    $(".logout").click(async function (event) {
        try {
            ...
            $(".btn-logged-out").show();
            //navigateTo("feed");
            window.location = 'https://100good.xyz';
        } catch (error) {
            console.error(error.message);
        }
    });
    ...
    <div id="post-${data.id}" class="feed-post bg-white shadow rounded-md dark:bg-gray-900 -mx-2 lg:mx-0">
        ...
        <div class="py-3 px-4 space-y-3"> 
            ...
            <div class="flex space-x-4 lg:font-bold">
                ...
                <a href="#" data-id="${data.id}" class="stream flex items-center space-x-2">
                    <div><i class="uil-dollar-sign mr-1" style="font-size: 130%;"></i><span class="stream-button-text">Stream</span></div>
                </a>
                ${mintHTML}
            </div>
        ...
        <div class="lg:w/8/12 flex-1 flex flex-col lg:items-start items-center"> 
            ...
            <p class="lg:text-left mb-2 text-center  dark:text-gray-100">${data.about} </p>
                ...
                <div class="capitalize flex font-semibold space-x-3 text-center text-sm my-2">
                    <a href="#" data-address="${data.address}" class="follow-button bg-pink-500 shadow-sm p-2 pink-500 px-6 rounded-md text-white hover:text-white hover:bg-pink-600"><i class="uil-dollar-sign mr-1" style=""></i> Follow</a>
                    <div>
...
function getSidebarUserHTML(data) {
    ...
    var followButton = `<a href="#" data-address="${data.address}" class="follow-button border border-gray-200 font-semibold px-4 py-1 rounded-full hover:bg-pink-600 hover:text-white hover:border-pink-600 dark:border-gray-800"><i class="uil-dollar-sign mr-1" style=""></i> Follow </a>`;
    if (loggedInUser) {
                ...
                <div class="story-content p-4" data-simplebarX>
                    ...
                    <div class="py-4 ">
                        <div class="flex justify-around">
                            ...
                            <a href="#" data-id="${data.id}" class="stream flex items-center space-x-4">
                                <div class="flex font-bold items-baseline"> <i class="uil-dollar-sign mr-1"></i> <span class="stream-button-text">Stream</span></div>
                            </a>
                            ${mintHTML}
                        </div>
                        <hr class="-mx-4 my-3">
